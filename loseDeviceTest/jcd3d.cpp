...
D3DPRESENT_PARAMETERS jcd3d::jcd3d_presentParams;
...
BOOL jcd3d::jcd3d_init(HINSTANCE hInstance, INT windowX, INT windowY, INT windowWidth, INT windowHeight, BOOL windowed, D3DDEVTYPE deviceType, DWORD maxTextureBlendStages)
{
	...
	{
		jcwin32_messageBoxErrorM("VertexShader version error");
		return FALSE;
	}
	...
	jcd3d_presentParams.BackBufferWidth = windowWidth;
	jcd3d_presentParams.BackBufferHeight = windowHeight;
	jcd3d_presentParams.BackBufferFormat = D3DFMT_A8R8G8B8;
	jcd3d_presentParams.BackBufferCount = 1;
	jcd3d_presentParams.MultiSampleType = D3DMULTISAMPLE_NONMASKABLE;
	jcd3d_presentParams.MultiSampleQuality = 0;
	jcd3d_presentParams.SwapEffect = D3DSWAPEFFECT_DISCARD;
	jcd3d_presentParams.hDeviceWindow = hwnd;
	jcd3d_presentParams.Windowed = windowed;
	jcd3d_presentParams.EnableAutoDepthStencil = TRUE;
	jcd3d_presentParams.AutoDepthStencilFormat = D3DFMT_D24S8;
	jcd3d_presentParams.Flags = 0;
	jcd3d_presentParams.FullScreen_RefreshRateInHz = D3DPRESENT_RATE_DEFAULT;
	jcd3d_presentParams.PresentationInterval = D3DPRESENT_INTERVAL_DEFAULT; // D3DPRESENT_INTERVAL_IMMEDIATE

	HRESULT hr = d3d9->CreateDevice(D3DADAPTER_DEFAULT, deviceType, hwnd, vp, &jcd3d_presentParams, &jcd3d_lpd3dd);
	if(FAILED(hr))
	{
		d3d9->Release();
...
VOID jcd3d::jcd3d_loop()
{
	...
	while(msg.message != WM_QUIT)
	{
		...
		else
		{
			...
			HRESULT presentH = jcd3d_lpd3dd->Present(NULL, NULL, NULL, NULL);
			if(presentH == D3DERR_DEVICELOST)
			{
				// device lose
				jccommon_trace1("%s\n", "device lose");
				// device waiting for reset
				if(jcd3d_lpd3dd->TestCooperativeLevel() == D3DERR_DEVICENOTRESET)
				{
					jccommon_trace1("%s\n", "device waiting for reset");
					jcd3d_lpd3dd->Reset(&jcd3d_presentParams);
				}
			}
			lastTime = currTime;
		}
