...
BOOL jcd3d::jcd3d_init(HINSTANCE hInstance, INT width, INT height, BOOL windowed, D3DDEVTYPE deviceType, DWORD maxTextureBlendStages, HWND pHwnd)
{
	if(pHwnd == NULL)
	{
		// 创建窗口
	...
	{
		MessageBox(NULL, L"MaxTextureBlendStages Failed", NULL, MB_OK);
		return FALSE;
	}
	...
	d3dpp.hDeviceWindow = jcd3d_hwnd;
	...
	HRESULT hr = d3d9->CreateDevice(D3DADAPTER_DEFAULT, deviceType, jcd3d_hwnd, vp, &d3dpp, &jcd3d_lpd3dd);
	if(FAILED(hr))
	{
		d3d9->Release();
...
VOID jcd3d::jcd3d_loop()
{
	...
	jcd3d_release();
	jcd3d_releaseHandle(&jcd3d_lpd3dd);
}
