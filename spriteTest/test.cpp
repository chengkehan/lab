...
BOOL GameInitialize()
{
	AdjustWindowRectEx(&windowRect, GetWindowExStyle(g_hwnd), GetMenu(g_hwnd) != NULL, GetWindowExStyle(g_hwnd));
	...
	if(pixelFormat != PIXEL_FORMAT_LEGAL)
	{
		MessageBox(NULL, L"Only support 32bit color screen.", L"Warning", MB_OK);
		return FALSE;
	}
...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE prevHInstance, LPSTR lpCmdline, INT nCmdshow)
{
	...
	g_hwnd = CreateWindowEx(NULL, WND_CLASS_NAME, WND_TITLE, WS_OVERLAPPED | WS_VISIBLE, WND_X, WND_Y, WND_WIDTH, WND_HEIGHT, NULL, NULL, hInstance, NULL);
	if (g_hwnd == NULL)
	{
		return 0;
	}
